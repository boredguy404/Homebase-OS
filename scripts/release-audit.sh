...
printf '%s' "$apps_json" | node -e '
  let raw="";process.stdin.on("data",chunk=>raw+=chunk).on("end",()=>{
    ...
    const expected=["Maker Desk","Quest Board","NovaShell World","Pokedex Browser","Weather Station","Focus Deck","Pantry Ledger","Loop Lab","Writer Desk","Reset Station","Pose Lab"];
