...
# mGBA accepts multiple content files as separate game windows.  Its Link menu
# then joins them using the real local-link implementation rather than two
# unrelated browser emulator tabs.
setsid -f env DISPLAY=:0 "$emulator" -3 "$rom" "$rom"

# Tile the two linked handheld screens side by side after they appear.
for attempt in $(seq 1 30); do
  mapfile -t windows < <(DISPLAY=:0 xdotool search --onlyvisible --name "mGBA\|$window_title" 2>/dev/null | tail -2)
  if [ "${#windows[@]}" -ge 2 ]; then
    ...
    notify-send "Pocket Archive multiplayer" "Two mGBA windows are ready. In the first: File → New Multiplayer Window, then choose Multiplayer/VS." 2>/dev/null || true
    exit 0
  fi
  sleep 0.4
done
