...
$('#audioFile').onchange=async event=>{const files=[...event.target.files].filter(file=>file.size>0);if(!files.length)return;localPlaylist=files;$('#status').textContent='LOCAL PLAYLIST · '+files.length+' TRACK'+(files.length===1?'':'S')+' LOADED';try{await playLocal(0)}catch{$('#status').textContent='PLAYLIST LOADED · TAP PLAY TO START'}};
