...
function updateNow(){
  const s=state.current; $('#stationName').innerHTML=s?`${esc(s.name)}<br><em>on air.</em>`:'Find your<br><em>frequency.</em>';
  ...
  $('#nowPlaying').textContent=s?s.name.toUpperCase():'NO SIGNAL SELECTED'; $('#location').textContent=s?(s.country||'WORLDWIDE').toUpperCase():'WORLDWIDE';
  $('#play').textContent=state.playing?'Ⅱ':'▶'; $('#favorite').textContent=s&&isFav(s)?'♥':'♡'; $('#favorite').classList.toggle('saved',!!s&&isFav(s));
  parent.postMessage({type:'orbit-state',name:s?.name||'Choose a signal',playing:state.playing},'*');
}
...
setInterval(()=>{const reactor=$('#beat-reactor');if(!reactor)return;const live=analyser&&state.playing;reactor.style.setProperty('--beat',(live?beat:.08).toFixed(3));reactor.style.setProperty('--bass',(live?bass:.08).toFixed(3));reactor.style.setProperty('--mids',(live?mids:.06).toFixed(3));reactor.classList.toggle('audio-live',!!live)},50);
function draw(t){
