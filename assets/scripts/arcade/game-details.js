(() => {
  ...
  const connectedPads = () => [...(navigator.getGamepads?.() || [])].filter(Boolean).sort((a,b)=>(a.index||0)-(b.index||0));
  async function open(card) {
    ...
    const versus=dialog.querySelector('.game-versus'),title=card.dataset.name||'',playerInfo=`${card.dataset.players||''} ${card.querySelector('.tag')?.textContent||''}`,knownN64=/goldeneye|mortal kombat|super smash|smash bros|mario kart 64|perfect dark|wcw|wwf|nba hangtime|nfl blitz|killer instinct|clayfighter|fighters destiny|fighter destiny|mace.*dark age|war gods|bio f\.r\.e\.a\.k\.s|rakuga kids|dark rift|dual heroes|ready 2 rumble|power stone/i.test(title),markedMultiplayer=/([2-4]|two|four).{0,10}player|multi|versus/i.test(playerInfo),n64Versus=core==='n64'&&(knownN64||markedMultiplayer),mode=/mario kart.*super circuit/i.test(title)?'mario':/street fighter/i.test(title)?'streetfighter':n64Versus?'local':null,padCount=connectedPads().length;versus.hidden=!mode;versus.textContent=mode==='local'?(padCount>1?'2 players · 2 pads ready':'2 players · connect P2'):'2 players';versus.onclick=()=>{if(mode==='local'&&connectedPads().length<2){toast('Wake both Xbox controllers, then tap 2 players again.');return}dialog.close();if(mode==='mario'||mode==='streetfighter'){window.startVersus?.(mode);return}sessionStorage.setItem('novashell-two-player-launch','1');window.launch(card.dataset.rom,card.dataset.name,card.dataset.core||'n64');setTimeout(()=>toast('P1 and P2 are assigned separately. Choose versus or multiplayer in the game menu.'),500)};
