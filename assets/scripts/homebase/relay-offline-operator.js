(()=>{
  ...
  const boot=()=>{
    ...
    const navigate=action=>{if(!action)return;const button=document.createElement('button');button.type='button';button.className='relay-offline-action';button.textContent=action.label||'Open';button.onclick=()=>{if(action.type==='navigate')parent.openPanel?.(action.target);else if(action.type==='system')parent.document.querySelector('.metric')?.click()};result.append(button)};
