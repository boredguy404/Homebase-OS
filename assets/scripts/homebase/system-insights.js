addEventListener('DOMContentLoaded', () => {
  document.head.insertAdjacentHTML('beforeend', '<link rel="stylesheet" href="/assets/styles/homebase/system-recovery.css">');
  ...
  if (status && !document.querySelector('#system-terminal-card')) {
    ...
    terminal.innerHTML='<span class="system-gyro" aria-hidden="true"><i></i><i></i><i></i><b>◈</b></span><span class="system-terminal-copy"><small>NOVASHELL · LOCAL SYSTEM</small><b>Reading this machine…</b><em>Tap for system activity, device readiness, storage, processes, and larger-file details.</em></span><span class="system-terminal-stats"><i>MEM —</i><i>DISK —</i><i>LOAD —</i></span><span class="system-terminal-open">OPEN ▸</span>';
    status.before(terminal);
    terminal.addEventListener('click',()=>document.querySelector('.metric')?.click());
  }
  document.head.insertAdjacentHTML('beforeend','<style>.legacy-system-metrics{position:absolute!important;width:1px!important;height:1px!important;overflow:hidden!important;clip:rect(0 0 0 0)!important;white-space:nowrap!important}.system-terminal-card{width:100%;display:grid;grid-template-columns:auto minmax(0,1fr) auto auto;gap:15px;align-items:center;margin:0 0 30px;padding:15px 18px;border:1px solid var(--line);border-radius:16px;background:linear-gradient(120deg,#0e191fdd,#172a2bca 55%,#101417dc);color:var(--text);text-align:left;cursor:pointer;box-shadow:inset 0 1px #ffffff1f,0 12px 30px #0003}.system-gyro{position:relative;display:grid;place-items:center;width:48px;height:48px;perspective:120px;transform-style:preserve-3d}.system-gyro i{position:absolute;width:33px;height:33px;border:1px solid rgb(var(--signal-alt-rgb));border-radius:50%;box-shadow:0 0 14px rgba(var(--signal-alt-rgb),.25);animation:systemGyro 4.2s linear infinite}.system-gyro i:nth-child(1){transform:rotateX(68deg)}.system-gyro i:nth-child(2){transform:rotateY(68deg);animation-delay:-1.4s}.system-gyro i:nth-child(3){transform:rotate(45deg) rotateX(68deg);animation-delay:-2.8s}.system-gyro b{position:relative;color:rgb(var(--signal-alt-rgb));font:18px ui-monospace,monospace;text-shadow:0 0 12px rgba(var(--signal-alt-rgb),.5)}.system-terminal-copy small,.system-terminal-copy b,.system-terminal-copy em{display:block}.system-terminal-copy small{color:rgb(var(--signal-alt-rgb));font:700 9px ui-monospace,monospace;letter-spacing:.14em}.system-terminal-copy b{margin:3px 0;font:700 16px ui-monospace,monospace}.system-terminal-copy em{color:var(--muted);font:11px Manrope,system-ui,sans-serif;font-style:normal}.system-terminal-stats{display:flex;gap:6px;flex-wrap:wrap;justify-content:flex-end}.system-terminal-stats i{padding:4px 6px;border:1px solid #ffffff24;border-radius:5px;color:#d8f1f4;font:10px ui-monospace,monospace;font-style:normal}.system-terminal-open{color:rgb(var(--signal-alt-rgb));font:700 10px ui-monospace,monospace;letter-spacing:.1em}@keyframes systemGyro{to{transform:rotateX(360deg) rotateY(360deg)}}html[data-theme="ultra-retro"] .system-terminal-card{border:2px solid #111;border-radius:0;background:#d4d4d4;color:#111;box-shadow:inset 2px 2px #fff,inset -2px -2px #666,4px 4px #111}html[data-theme="ultra-retro"] .system-gyro i{border:2px solid #000080;border-radius:0;box-shadow:2px 2px #111}html[data-theme="ultra-retro"] .system-gyro b{color:#000080;text-shadow:none}html[data-theme="ultra-retro"] .system-terminal-copy small,html[data-theme="ultra-retro"] .system-terminal-open{color:#000080}html[data-theme="ultra-retro"] .system-terminal-copy em{color:#222}html[data-theme="ultra-retro"] .system-terminal-stats i{border:1px solid #555;border-radius:0;background:#eee;color:#111}@media(max-width:720px){.system-terminal-card{grid-template-columns:auto 1fr;gap:9px}.system-terminal-stats{justify-content:flex-start;grid-column:2}.system-terminal-open{display:none}}@media(prefers-reduced-motion:reduce){.system-gyro i{animation:none}}</style>');
  const cards = [...document.querySelectorAll('.metric')];
  const dialog = document.createElement('dialog');
  dialog.className = 'insight-dialog';
  dialog.innerHTML = '<button class="insight-x">×</button><small>LIVE · TAP ANY STATUS CARD</small><h2>System activity</h2><p id="insight-rating">Reading this Chromebook…</p><div id="live-facts" class="live-facts"></div><section id="readiness" class="readiness"><h3>Device readiness</h3><p>Turning local measurements into safe, useful next steps…</p></section><section id="recovery" class="recovery-coach"><h3>Cleanup &amp; recovery coach</h3><p>Read-only guidance. NovaShell never deletes anything from this report.</p><div class="recovery-grid"></div></section><section id="storage-composition"><h3>Storage composition</h3><div id="composition" class="composition"></div></section><section id="running-processes"><h3>Real running processes</h3><p class="explain">Sorted by CPU use. Read-only for touch safety.</p><div id="processes" class="insight-list skeleton-list"></div></section><section id="largest-files"><h3>Largest personal files</h3><div id="largest" class="insight-list skeleton-list"></div></section>';
  ...
  function paintRecovery(data) {
    const recovery=data.recovery||{},stale=recovery.stale_downloads||{},trash=recovery.trash||{},backup=recovery.backup||{},grid=dialog.querySelector('.recovery-grid');
    const last=backup.last_export?new Date(backup.last_export*1000):null,days=last?Math.floor((Date.now()-last.getTime())/86400000):null;
    const cards=[
      {mark:stale.count?'!':'✓',title:stale.count?stale.count+' older download'+(stale.count===1?'':'s'):'Downloads look current',detail:stale.count?human(stale.bytes)+' has not changed in '+stale.age_days+'+ days. Review it before removing anything.':'No files older than '+(stale.age_days||90)+' days were found.',label:'Review Downloads',url:'/pages/files.html?path=Downloads'},
      {mark:trash.count?'↶':'✓',title:trash.count?trash.count+' recoverable Trash item'+(trash.count===1?'':'s'):'Trash is empty',detail:trash.count?human(trash.bytes)+' remains recoverable. Review it in Library; this coach never empties Trash.':'There is nothing waiting in the local Trash folder.',label:'Review Trash',url:'/pages/files.html?path=.local%2Fshare%2FTrash%2Ffiles'},
      {mark:last&&days<30?'✓':'!',title:last?'Last backup '+(days===0?'today':days+'d ago'):'No NovaShell backup recorded',detail:last?human(backup.bytes)+' · '+(backup.areas.length?backup.areas.join(', '):'preferences only')+'. This records a completed local download, not an upload.':'Export selected data to create a portable local archive. Credentials remain excluded.',label:'Backup settings',url:'/pages/settings.html?panel=backup'}
    ];
    grid.replaceChildren(...cards.map(item=>{const card=document.createElement('article'),mark=document.createElement('i'),copy=document.createElement('span'),title=document.createElement('b'),detail=document.createElement('span'),button=document.createElement('button');mark.textContent=item.mark;title.textContent=item.title;detail.textContent=item.detail;button.type='button';button.textContent=item.label;button.onclick=()=>go(item.url);copy.append(title,detail);card.append(mark,copy,button);return card}));
  }
  async function refresh() {
    ...
    const view=Number(dialog.dataset.view||2);dialog.querySelector('#insight-rating').textContent = view===0?'Review storage, stale Downloads, recoverable Trash, and the last completed local backup—nothing is deleted here.':view===1?'Memory usage is live; this view focuses on the processes using resources.':rating + ' · ' + load.toFixed(2) + ' load across ' + data.cpu_count + ' CPU threads';
  ...
  async function open(index = 2) { const views=[['Storage & recovery','Storage composition, larger files, recoverable Trash, stale Downloads, and backup status.'],['Memory details','Live memory pressure and the processes using the most resources.'],['System load details','Live CPU load, uptime, and the busiest current processes.']];const view=views[index]||views[2];dialog.querySelector('h2').textContent=view[0];dialog.querySelector('#insight-rating').textContent=view[1];dialog.dataset.view=index;dialog.showModal();await refresh();const storage=index===0,memory=index===1;dialog.querySelector('#readiness').hidden=memory;dialog.querySelector('#recovery').hidden=!storage;dialog.querySelector('#storage-composition').hidden=!storage;dialog.querySelector('#running-processes').hidden=storage;dialog.querySelector('#largest-files').hidden=!storage; }
  cards.forEach(card => {
    card.tabIndex = 0;
  ...
  refresh().catch(()=>{});
  const captureView=new URLSearchParams(location.search).get('capture');
  if (captureView === 'system') setTimeout(() => open(2), 180);
  if (captureView === 'recovery') setTimeout(() => open(0), 180);
});
