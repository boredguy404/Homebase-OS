...
(()=>{
  ...
  const boot=()=>{
    ...
    const log=document.createElement('section');log.className='relay-build-log';log.innerHTML='<h3>RELAY BUILD CHAT · ONE WAY</h3><p>I post brief implementation updates here while work moves. It refreshes automatically; you never need to reload this page.</p><div class="relay-build-events" aria-live="polite" aria-relevant="additions text"><div class="relay-build-event"><time>…</time><span>Connecting to local build feed…</span></div></div>';
    board.append(log);
    const render=feed=>{
      mergeTasks(feed.tasks);
      projectStamp.textContent='RELAY BUILD FEED · updated '+stamp(feed.updated)+' · live every 1.2s';
    ...
    const poll=async()=>{try{const response=await fetch('/api/project-feed',{cache:'no-store'});if(!response.ok)throw Error();render(await response.json())}catch{projectStamp.textContent='RELAY BUILD FEED · local server unavailable'}finally{setTimeout(poll,1200)}};
    poll();
